...
// класс Notebook -главный класс программы
public class Phonebook 
{
    ...
    String fileName;
    // Диалоговое окно для добавления/редактирвоания записи
    EditDialog edit;
    ...
    // Сама таблицы
    JTable mainTable;
    // Модель выделения записей в таблице
    ...
    public Phonebook()
    {
        ...
        mainTm.addColumn("Адрес");
        mainTm.addColumn("Дополнительная информация");
            
        // Устанавливаем для таблицы модель
    ...
    Action createAction = new AbstractAction("Создать")
    {
       public void actionPerformed(ActionEvent event)
       {
          ...
          all.clear();
          fileName=null;
          JOptionPane.showMessageDialog(f, "Новый справочник создан");
       }
        };
              ...
              try 
              {
                   ...
                   //Десериализация файла
                    Load(fileName);
                      ...
                      {
                            // Заполняем таблицу
                            mainTm.addRow(new String[] {r.getName(),r.getNomberphone(),r.getEmail(),r.getAdress(),r.getAdditionalinformation()});
                      }
                } 
    ...
    Action exitAction = new AbstractAction("Выйти")
    {
       public void actionPerformed(ActionEvent event)
       {
           //получаем выбранное пользователем занчение по поводу сохранения
            int response = JOptionPane.showConfirmDialog(f,"Сохранить текущий справочник?", 
                    "Сохранить текущий справочник?",JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
           ...
           public void actionPerformed(ActionEvent event)
           {
               ...
               if(ind>-1)
               {
                   ...
                   Record tempRecord = new Record ();
                    ...
                    tempRecord.setNomberphone(mainTable.getValueAt(ind,1).toString());
                    tempRecord.setEmail(mainTable.getValueAt(ind,2).toString());
                    tempRecord.setAdress(mainTable.getValueAt(ind,3).toString());
                    tempRecord.setAdditionalinformation(mainTable.getValueAt(ind, 4).toString());
        ...
        Action removeAction = new AbstractAction("Удалить")
        {
           public void actionPerformed(ActionEvent event)
           {
               ...
               if(ind>-1)
               {
                ...
                oldRecord.setNomberphone(mainTable.getValueAt(ind,1).toString());
                oldRecord.setEmail(mainTable.getValueAt(ind,2).toString());
                oldRecord.setAdress(mainTable.getValueAt(ind,3).toString());
                oldRecord.setAdditionalinformation(mainTable.getValueAt(ind, 4).toString());
...
class EditDialog extends JDialog
{
  public EditDialog(JFrame owner, String title, final Record oldRecord)
  {
    ...
    JLabel lab1=new JLabel("Имя :");
     ...
     JLabel lab2=new JLabel("Номер телефона :");
     ...
     JLabel lab4=new JLabel("Адрес :");
        lab4.setBounds(10, 100, 50, 20);
        lab4.setFont(new Font("Arial",0,11));
     JLabel lab5=new JLabel("Дополнительная информация :");
        
     final JTextField nameField = new JTextField();
     nameField.setBounds(100, 10, 200, 20);
     
     final JTextField nomberField = new JTextField();
     nomberField.setBounds(100, 40, 200, 20);
     
     final JTextField emailField = new JTextField();
     emailField.setBounds(100, 70, 200, 20);
     
     final JTextField adressField = new JTextField();
     adressField.setBounds(100, 100, 200, 20);
     
     final JTextField additionalFiled = new JTextField();
     additionalFiled.setBounds(100, 100, 200, 20);
    ...
    ok.addActionListener(new ActionListener()
    {
      public void actionPerformed(ActionEvent event)
      {
         ...
         try
         {
          ...
          //проверяемм коректность введенного имени
          if(testName(nameField.getText())==false)
          {
              throw new Exception("Некорректно введено имя");
          }
          //проверяемм коректность введенной фамилии
          if(testName(nameField.getText())==false)
          {
          ...
          tempRec.setNomberphone(nomberField.getText());
          tempRec.setEmail(emailField.getText());
          tempRec.setAdress(adressField.getText());
          tempRec.setAdditionalinformation(additionalFiled.getText());
         ...
         if(compare(tempRec)==false)
         {
             ...
             mainTm.addRow(new String[]{tempRec.getName(),tempRec.getNomberphone(), 
                 tempRec.getEmail(), tempRec.getAdress(), tempRec.getAdditionalinformation()});
    ...
    if(oldRecord!=null)
    {
        ...
        nomberField.setText(oldRecord.getNomberphone());
        emailField.setText(oldRecord.getEmail());
        adressField.setText(oldRecord.getAdress());
        additionalFiled.setText(oldRecord.getAdditionalinformation());
    }
    
    // создаем панель и добавляем на нее все кнопки
    JPanel panel = new JPanel();
    panel.setLayout(null);
    panel.add(ok);
    panel.add(delet);
    panel.add(nameField);
    panel.add(nomberField);
    panel.add(emailField);
    panel.add(adressField);
    panel.add(additionalFiled);
    panel.add(lab1);
    panel.add(lab2);
    panel.add(lab3);
    panel.add(lab4);
...
class FindDialog extends JDialog
{
  ...
  public FindDialog(JFrame owner, String title)
  {
    ...
    JLabel lab1=new JLabel("Ключ поиска :");
     ...
     String options[] = {"По имени", "По номеру телефона", "По email", "По адресу"};
      ...
      public void actionPerformed(ActionEvent event)
      {
		...
		{		
                    ...
                    if(r.getName().equals(string4find))
                    {
                        ...
                        tm.addRow(new String[] {r.getName(),r.getNomberphone(),r.getEmail(),r.getAdress(), r.getAdditionalinformation()});
                    }
		}
             }
             else if(mode==1)
             {
                 for(Record r : all)
		{		
                    if(r.getNomberphone().equals(string4find))
                    {
                        tm.addRow(new String[] {r.getName(),r.getNomberphone(),r.getEmail(),r.getAdress(), r.getAdditionalinformation()});
                    }
		}
		...
		{		
             ...
             else if(mode==3)
             {
                 for(Record r : all)
		{		
                    if(r.getAdress().equals(string4find))
                    {
                        tm.addRow(new String[] {r.getName(),r.getNomberphone(),r.getEmail(),r.getAdress(), r.getAdditionalinformation()});
                    }
		}
             ...
             else if(mode==4)
             {
                 for(Record r : all)
                 {
                     if (r.getAdditionalinformation().equals(string4find))
                     {
                         tm.addRow(new String[] {r.getName(),r.getNomberphone(),r.getEmail(),r.getAdress(), r.getAdditionalinformation()});
                     }
                 }
            ...
            // добавляем в таблицу четыре столбца
            tm.addColumn("Имя");
            tm.addColumn("Номер телефона");
            tm.addColumn("E-mail");
            tm.addColumn("Адрес");
            tm.addColumn("Дополнительная информация");
            ...
            panel.add(scrol);
            panel.add(modes);
            // а ппанель добавляем в диалоговое окно
            add(panel, BorderLayout.CENTER);
            setSize(550, 220);
  }
